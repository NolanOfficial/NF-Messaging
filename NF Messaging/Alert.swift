...
struct Alert {
    ...
    static func appearance() {
        ...
        pv.titleFont    = UIFont(name: "HelveticaNeue-Light", size: 22)!
        pv.titleColor   = .white
    ...
    static func showBasicAlert(on vc: UIViewController, title: String, message: String) {
        ...
        let cancelButton = CancelButton(title: "Try Again") {
    ...
    static func showSuccessAlert(on vc: UIViewController, title: String, message: String) {
        let errorImage = UIImage(named: "Success Error.jpg")
        let alert = PopupDialog(title: title, message: message, image: errorImage!)
        let cancelButton = CancelButton(title: "Okay") {
            print("Alert Cancelled")
        }
        alert.addButton(cancelButton)
        DispatchQueue.main.async {
